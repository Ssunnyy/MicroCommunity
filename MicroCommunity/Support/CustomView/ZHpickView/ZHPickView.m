...
-(instancetype)initPickviewWithIsHaveNavControler:(BOOL)isHaveNavControler {
    self=[super init];
    if (self) {
         [self setUpPickView];
        [self setFrameWith:isHaveNavControler];
...
- (void) configWithDataArray:(NSArray *) array{
...
-(void)setArrayClass:(NSArray *)array{
    if (_dicKeyArray.count > 0) {
        [_dicKeyArray removeAllObjects];
    }else {
...
-(void)setUpPickView{
    ...
    [pickView setBorderWidth:1 andBorderColor:RGBCOLOR(215, 215, 215)];
    pickView.alpha = 0.9;
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
    ...
    [datePicker setBorderWidth:1 andBorderColor:RGBCOLOR(215, 215, 215)];
    if (_defaulDate) {
...
-(void)remove{
    ...
    CGFloat toolViewH = _pickeviewHeight+ZHToobarHeight;
    float toolViewY= [UIScreen mainScreen].bounds.size.height-toolViewH;
    [UIView animateWithDuration:.3f animations:^{
        self.frametop = SCREEN_HEIGHT + toolViewY;
    }completion:^(BOOL finished) {
        [self removeFromSuperview];
    }] ;
    
    if([self.delegate respondsToSelector:@selector(toolNoData)]){
        [self.delegate toolNoData];
    }
}
...
-(void)doneClick
{
    if (_pickerView) {
        ...
        _resultString= [self dateToString:_datePicker.date];
        
    }
    
    if (_resultString.length == 0) {
        _resultString = [_plistArray objectAtIndex:currentRow];
